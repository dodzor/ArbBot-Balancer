...
const determineProfitability = async (_routerPath, _token0Contract, _token0, _token1) => {
  ...
  console.log('uReserves: ' + ethers.formatUnits(uReserves[1], 'ether') + ' ETH')
  console.log('sReserves: ' + ethers.formatUnits(sReserves[1], 'ether') + ' ETH' + '\n')

  let minAmount
  let divider = 100;

  if (uReserves[0] > sReserves[0]) {
    minAmount = BigInt(sReserves[0]) / BigInt(divider)
  } else {
    minAmount = BigInt(uReserves[0]) / BigInt(divider)
  }
